...
def run_inference() -> None:
    ...
    configure_project("real_time_prediction")
    ...
    accumulated_preds: list[pd.DataFrame] = []
    ...
    for step in range(num_steps):
        ...
        # Run pipeline (saves full batch predictions to predictions_path)
        with KedroSession.create(project_path=project_path) as session:
            session.run(pipeline_name="inference")

        # Extract only the newest prediction (last row) and accumulate
        batch_preds = pd.read_csv(predictions_path)
        batch_preds["datetime"] = pd.to_datetime(batch_preds["datetime"])
        accumulated_preds.append(batch_preds.iloc[[-1]])
        pd.concat(accumulated_preds, ignore_index=True).to_csv(predictions_path, index=False)
